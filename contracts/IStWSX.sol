...
abstract contract IStWSX is IERC20 {
     ...
     *
     * @dev Always equals to `_getTotalPooledWSX()` since token amount
     * is pegged to the total amount of WSX controlled by the protocol.
     */
    function totalSupply() external view returns (uint256) {
        return _getTotalPooledWSX();
    }

    /**
     * @return the entire amount of WSX controlled by the protocol.
     *
     * @dev The sum of all WSX balances in the protocol, equals to the total supply of stWSX.
     */
    function getTotalPooledWSX() external view returns (uint256) {
        return _getTotalPooledWSX();
    }
     ...
     * total WSX controlled by the protocol. See `sharesOf`.
     */
    function balanceOf(address _account) external view returns (uint256) {
        return getPooledWSXByShares(_sharesOf(_account));
    }
     ...
     * - the caller must have allowance for `_sender`'s tokens of at least `getPooledWSXByShares(_sharesAmount)`.
     * - the contract must not be paused.
     *
     * @dev The `_sharesAmount` argument is the amount of shares, not tokens.
     */
     ...
     */
    function _getTotalPooledWSX() internal view virtual returns (uint256);
     ...
     * - `_recipient` cannot be the zero address or the `stWSX` token contract itself
    ...
    function _burnShares(
        ...
        uint256 preRebaseTokenAmount = getPooledWSXByShares(_sharesAmount);
